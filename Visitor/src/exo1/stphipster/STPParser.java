...
public class STPParser {
	...
	/**
	 * Convert a text command to the corresponding object in the the {@link exo1.lib.STPCommand}.
	 * If the format of cmd is invalid, returns Optional.empty()
	 * @param cmd
	 *         the line to be parsed. Must be non-null.
	 * @return the corresponding {@link exo1.lib.STPCommand} wrapped in a {@link Optional}
	 */
	public static Optional<STPCommand> parse(String cmd){
		...
		case "elapsed": 
			...
			var timersId = Arrays.stream(tokens).skip(1).map(Integer::parseInt).toList();
